...
setup-hooks: ## Configure git pre-commit hook path (.githooks/)
	@echo -e "$(CYAN)--> Configuring git hooks to .githooks/...$(RESET)"
	@git config core.hooksPath .githooks
	@chmod +x .githooks/pre-commit
	@echo -e "$(GREEN)✓ Git pre-commit hook configured (.githooks/pre-commit).$(RESET)"

pre-commit: ## Run pre-commit validations
